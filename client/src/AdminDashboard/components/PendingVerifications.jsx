...
function PendingVerifications() {
  ...
  const fetchPendingBrands = async () => {
    try {
      ...
      const res = await axios.get('https://micromatch.onrender.com/api/brands/pending', {
        headers: {
          'x-auth-token': token,
        }
      });
  ...
  const handleApprove = async (id) => {
    try {
      const token = localStorage.getItem('token');
      await axios.put(`https://micromatch.onrender.com/api/brands/approve/${id}`, {}, {
  ...
  const handleReject = async (id) => {
    try {
      const token = localStorage.getItem('token');
      await axios.delete(`https://micromatch.onrender.com/api/brands/reject/${id}`, {
        headers: {'x-auth-token': token},
      });
      setBrands((prev) => prev.filter((brand) => brand._id !== id));
    } catch (err) {
